...
def init(workdir: str,
    ...
    assign_df[[asn_mid_col, asn_gid_col, reason_col, g_prop_col]] = ['unassigned', 'unassigned', 'unassigned', '']
...
def mp_prop_gauges(df: pd.DataFrame, n_processes: int or None = None) -> pd.DataFrame:
    ...
    gauged_mids = df[df[gid_col].notna()][mid_col].values[:15]

    logger.info('Assigning by Hydraulic Connectivity')
    with Pool(n_processes) as p:
        logger.info('Finding Downstream Assignments')
        df_prop_down = pd.concat(p.starmap(_map_propagate, [(df, x, 'down') for x in gauged_mids]))
        logger.info('Finding Upstream Assignments')
        df_prop_up = pd.concat(p.starmap(_map_propagate, [(df, x, 'up') for x in gauged_mids]))
        logger.info('Resolving Propagation Assignments')
        df_prop = pd.concat([df_prop_down, df_prop_up]).reset_index(drop=True)
        df_prop = pd.concat(p.starmap(_map_resolve_props, [(df_prop, x) for x in df_prop[mid_col].unique()]))
...
def _map_propagate(df: pd.DataFrame, start_mid: int, direction: str) -> pd.DataFrame or None:
    """
    ...
    try:
        while True:
            ...
            new_row[[asn_mid_col, asn_gid_col, g_prop_col]] = [start_mid, start_gid,
            ...
            if int(stream_row[down_mid_col].values[0]) == -1 or n_steps >= 12:
                break
...
def _map_resolve_props(df_props: pd.DataFrame, mid: str) -> pd.DataFrame:
    """
    ...
    df_mid[['direction', 'n_steps']] = pd.DataFrame(df_props[g_prop_col].apply(lambda x: x.split('-')[1:3]).to_list())
