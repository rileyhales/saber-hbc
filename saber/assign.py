...
def mp_assign_ungauged(df: pd.DataFrame) -> pd.DataFrame:
    ...
    with Pool(get_state('n_processes')) as p:
        ...
        for cluster_number in range(df[COL_CID].max() + 1):
            ...
            gauge_clstr = pd.read_csv('/path to/gauge /cluster/dtw_minimum_distances_summary.csv')
            c_df = df[df[COL_CID] == cluster_number]
            g_df = c_df[c_df[COL_GID].notna()]
            g_df = g_df.merge(gauge_clstr[['File', 'clst_gauge']], left_on='gauge_id', right_on='File', how='inner').drop(
                columns=['File'])
            # keep a list of the unassigned basins in the cluster
            mids = c_df[c_df[COL_ASN_REASON] == 'unassigned'][COL_MID].values
            # Split mids into chunks of 1,000
            chunks = [mids[i:i + 1000] for i in range(0, len(mids), 1000)]
            for i, chunk in enumerate(chunks):
                chunk_df = pd.concat(
                    p.starmap(_map_assign_ungauged, [(df, g_df, mid) for mid in chunk])
                )
            # df = pd.concat([
            #     pd.concat(p.starmap(_map_assign_ungauged, [(df, g_df, x) for x in mids])),
            #     df[~df[COL_MID].isin(mids)]
            # ]).reset_index(drop=True)
                chunk_df.to_csv(f'path/to/save/thechunk/mp_assign_ungauged_{cluster_number}_chunk_{i}.csv')
                print(f'mp_assign_ungauged is saved for cluster number {cluster_number}, chunk{i}')
    return df
...
def _map_assign_ungauged(assign_df: pd.DataFrame, gauges_df: pd.DataFrame, mid: str) -> pd.DataFrame:
    ...
    try:
        ...
        if (new_row[COL_GPROP].values[0] != '' and new_row[COL_RPROP].values[0] != '' and new_row[COL_RID].values[0]
                is not None):
        ...
        # if only regid and gprop are there in assigntable
...
def assign_nearest_cluster(new_row, assign_df, gauges_df, mid):
    ...
    gauge_clstr_num = new_row['clstr_id'].values[0]
    cluster_filter = (gauges_df[COL_CID] == cluster_num) & (
        gauges_df['strahler_order'].between(strahler_order - 1, strahler_order + 1) & (gauges_df['clst_gauge'] == gauge_clstr_num))
