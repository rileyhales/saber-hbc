...
if __name__ == "__main__":
    ...
    saber.cluster.cluster(workdir, x=x_fdc_train)
    ...
    saber.gis.clip_by_assignment(workdir, assign_df, drain_gis)
    saber.gis.clip_by_cluster(workdir, assign_df, drain_gis)
    saber.gis.clip_by_unassigned(workdir, assign_df, drain_gis)

    # Compute the corrected simulation data
    logger.info('Computing Bias Corrected Simulations')
    hdf = saber.io.read_table(workdir, 'hindcast_table')
    with Pool(20) as p:
        p.starmap(
            saber.calibrate.map_saber,
            [[mid, asgn_mid, asgn_gid, hdf, gauge_data] for mid, asgn_mid, asgn_gid in
