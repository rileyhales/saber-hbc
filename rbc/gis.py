...
def clip_by_assignment(atable: str, drain_shape: str, workdir: str, prefix: str = '') -> None:
    """
    Creates geojsons in workdir/gis_outputs
    ...
    for reason in reasons:
        selected_segments = dl[dl['COMID'].isin(atable[atable[reason_col] == reason][model_id_col].tolist())]
        name = f'{prefix}{"_" if prefix else ""}assignments_{reason}.json'
        selected_segments.to_file(os.path.join(workdir, 'gis_outputs', name), driver='GeoJSON')
    return
...
def clip_by_ids(ids: list, drain_shape: str, workdir: str, prefix: str = '') -> None:
    ...
    dl[dl[model_id_col].isin(ids)].to_file(os.path.join(workdir, 'gis_outputs', name), driver='GeoJSON')
    return
